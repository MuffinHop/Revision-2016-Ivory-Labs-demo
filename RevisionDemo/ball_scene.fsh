...
float getColorReflection( vec3 position, out vec3 color, out float reflectiveValue) {
    ...
    dist2 = sdSphere(_position3-vec3(4.,iGlobalTime/12.,1.),0.5+iGlobalTime/8.);
    dist2 -= noise(_position.xy*2.+iGlobalTime/8.+_position.z)/24.;
    if(dist2<dist) {
        dist=dist2;
...
vec4 march(vec3 rayOrigin, vec3 rayDirection) {
    ...
    float distanceOfFirstHit = 0.;
        ...
        for(float i = 0.; i < MAX_STEPS; i++) {
            ...
            if(length(curPosition*vec3(1.,6.,1.))>12.0) break;
           	/*too far from begining point, call it an end*/
            if( abs( stepable) < epsilon + i*i/5000.) {
                //if( reflection == 0.) {
...
vec4 render( vec2 uv ) {  
     float aspect = iResolution.x / iResolution.y;
        vec3 cameraPosition =  cam_dist;
    	float T=iGlobalTime*0.4;
    	float vieteri = cos(T)/(1.+T*T*T);
        vec3 direction = normalize( vec3(.5 * uv * vec2( aspect, 1.0), 0.6+vieteri) );
