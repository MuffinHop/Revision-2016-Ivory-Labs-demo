...
float getColorReflection( vec3 position, out vec3 color, out float reflectiveValue) {
    ...
    float heightmap = (1.+0.1*length(position.xz))*noise( _position.xz/5.)/(1.+dist_from_ball/12.);
    ...
    dist += (noise( _position.xy*1.121241)*sin(_position.x))/3.;
    
    
    
    
    float a=heightmap;
    	color = vec3(.45) - 2.*vec3(.4,.6,.8)*noise( _position.xz/4.)*noise( _position.xz*11.);
        color = min(max(color,vec3(0.)),vec3(1.));
    color += noise( _position.xz*55.);
    reflectiveValue = 0.2;
    float mickey = (noise(_position.xz*11.)+noise(_position.xz*44.))/200.;
    dist += mickey;
    color += mickey*33.;
    return dist;
}
...
vec4 march(vec3 rayOrigin, vec3 rayDirection) {
    ...
    float distanceOfFirstHit = 0.;
        ...
        for(float i = 0.; i < MAX_STEPS; i++) {
            ...
            if(distance(curPosition*vec3(1.0,3.0,1.0),rayOrigin*vec3(1.0,3.0,1.0))>12.0)break;
;           	/*too far from begining point, call it an end*/
            if( abs( stepable) < epsilon + i*i*i/500000. - 0.4/(1.+i)) {
                //if( reflection == 0.) {
...
vec4 render( vec2 uv ) {  
     float aspect = iResolution.x / iResolution.y;
        ...
        vec3 cameraPosition = vec3( sin(iGlobalTime)*1.8, .2, -iGlobalTime*1.1);
        float heightmap = noise( cameraPosition.xz/5.);
    	cameraPosition.y = 0.5+heightmap+vieteri*2.0;
        vec3 direction = normalize( vec3(.5 * uv * vec2( aspect, 1.0), field_of_view) );
        direction *= rotationMatrix(vec3(0.0,0.0,1.0), sin(iGlobalTime*1.523)/4.);
        direction *= rotationMatrix(vec3(1.0,0.0,0.0), sin(iGlobalTime*1.131)/7.+0.2+vieteri);
        direction *= rotationMatrix(vec3(0.0,1.0,0.0), sin(iGlobalTime*1.131)*0.8);
        direction.z *= -1.0;
